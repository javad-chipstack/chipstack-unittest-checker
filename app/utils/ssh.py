...
def check_passwordless_ssh(run_id: str, outdir_str) -> bool:
    ...
    try:
        ...
        if eda_server_ssh_port:
            ssh_command = f"ssh -p {eda_server_ssh_port} {eda_server_ssh_login_info} exit > /dev/null 2>&1"
        else:
            ssh_command = f"ssh {eda_server_ssh_login_info} exit > /dev/null 2>&1"
    ...
    try:
        if eda_server_ssh_port:
            scp_command = f"rsync -av -e \"ssh -p {eda_server_ssh_port}\" --exclude='**/xcelium.d/' --exclude='**/cov_work/' -e 'ssh -p {eda_server_ssh_port}' {eda_server_ssh_login_info}:/tmp/eda/{run_id} {outdir_str} > /dev/null 2>&1"
        else:
            scp_command = f"rsync -av --exclude='**/xcelium.d/' --exclude='**/cov_work/' {eda_server_ssh_login_info}:/tmp/eda/{run_id} {outdir_str} > /dev/null 2>&1"
