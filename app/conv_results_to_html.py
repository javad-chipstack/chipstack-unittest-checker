...
def main():
    output_dir = find_latest_simulation_dir("/Users/javad/dev/chipstack-ai")
    if not output_dir:
        raise FileNotFoundError("No simulation directory found.")
    ...
    final_report_path = os.path.join(output_dir, "index.html")
    with open(final_report_path, "w") as file:
        file.write(html_report)

    print(f"Done! Open {final_report_path} in a browser to view the reports.")


def find_latest_simulation_dir(base_dir):
    pattern = os.path.join(base_dir, "kpi/out/dev_v3/output_*/Simulation/")
    dirs = sorted(glob.glob(pattern), key=os.path.getmtime, reverse=True)
    return dirs[0] if dirs else None


def replace_name_with_link(file_path, base_url):
    with open(file_path, "r") as file:
        content = file.readlines()

    # Pattern to match the line and capture the name part
    pattern = re.compile(
        r'(<span class="w">\s*</span><span class="nt">&quot;name&quot;</span><span class="p">:</span><span class="w"> </span><span class="s2">&quot;)([^&]+)(&quot;</span><span class="p">,)'
    )

    with open(file_path, "w") as file:
        for line in content:
            # Replace name with hyperlink
            new_line = re.sub(
                pattern, r'\1<a href="tests/test_\2/report.html">\2</a>\3', line
            )
            file.write(new_line)
...
def process_test_directory(test_dir):
    ...
    # Step 4: Replace name with hyperlink in scenarios.json.html
    scenarios_file_path = os.path.join(test_dir, "scenarios.json.html")
    if os.path.exists(scenarios_file_path):
        replace_name_with_link(scenarios_file_path, "tests/")
...
def sv_to_html(sv_file_path: str, output_dir: str) -> None:
    ...
    html_report = generate_sv_html_report(sv_code, title=file_name)
...
def convert_metadata_to_html(metadata_file_path: str, test_dir: str) -> None:
    ...
    for test in data.get("logs", []):
        task_names = [
            ...
            if not match.startswith("primitive_")
        ]
...
def generate_html_report(test, task_names):
    ...
    for sim_step_idx, item in enumerate(test):
        ...
        highlighted_code = highlight(item["response"], VerilogLexer(), HtmlFormatter())
